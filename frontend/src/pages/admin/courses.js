...
const AdminCoursesTab = () => {
  ...
  const fetchCourses = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:4000/api/course/getAllCourses'); // Adjust this endpoint to match your backend
  ...
  const handleSubmitCourse = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:4000/api/course/addCourse', {
        method: 'POST',
        headers: {
  ...
  // Add this state for editing
    ...
    const handleUpdateCourse = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:4000/api/course/modifyCourse/${editCourse._id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
