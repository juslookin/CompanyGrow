...
const BonusSuccess = () => {
  ...
  const fetchSessionDetails = async (sessionId) => {
    try {
      ...
      const url = `http://localhost:4000/api/payment/session/${sessionId}`;
      console.log('🔍 Making request to:', url);
      const response = await fetch(`http://localhost:4000/api/payment/session/${sessionId}`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (sessionData.metadata.badgeIds) {
            ...
            await fetch('http://localhost:4000/api/payment/approve-badges', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
